...
class TestProtonSend:
    ...
    def test_get_recipient_keys_found(self, sender, mock_session):
        ...
        assert len(result) == 2
        mock_session.api_request.assert_called_once_with(
            "/mail/v4/keys/recipient@proton.me",
            method="get",
            params=None,
            jsondata=None
        )
    ...
    def test_get_recipient_keys_not_found(self, sender, mock_session):
        ...
        mock_session.api_request.return_value = {"Keys": []}
    ...
    def test_create_draft_success(self, sender, mock_session):
        ...
        mock_session.api_request.assert_called_once()
    ...
    def test_create_draft_with_thread_id(self, sender, mock_session):
        ...
        assert call_args[1]['jsondata']['Message']['ThreadID'] == "thread456"

    def test_create_draft_failure(self, sender, mock_session):
        """Test: Create draft fails."""
        mock_session.api_request.return_value = {"Message": {}}
    ...
    def test_update_draft_success(self, sender, mock_session):
        ...
        mock_session.api_request.return_value = {"Code": 1000}

        result = sender.update_draft("draft123", "encrypted body")

        assert result == True
        mock_session.api_request.assert_called_once_with(
            "/mail/v4/messages/draft123",
            method="put",
            params=None,
            jsondata={"Message": {"Body": "encrypted body", "MIMEType": "text/html"}}
        )

    def test_update_draft_failure(self, sender, mock_session):
        """Test: Update draft fails."""
        mock_session.api_request.side_effect = Exception("API error")
    ...
    def test_send_draft_success(self, sender, mock_session):
        ...
        assert result == True
        mock_session.api_request.assert_called_once_with(
            "/mail/v4/messages/draft123/send",
            method="post",
            params=None,
            jsondata=None
        )
    ...
    def test_send_draft_failure(self, sender, mock_session):
        ...
        mock_session.api_request.side_effect = Exception("Send failed")
    ...
    def test_send_email_full_flow_success(self, mock_cooldown, sender, mock_session, mock_crypto):
        ...
        mock_session.api_request.side_effect = [
            {"Keys": []},                       # get_recipient_keys
            {"Message": {"ID": "draft123"}},    # create_draft
            {"Code": 1000},                     # update_draft
            {"Code": 1000},                     # send_draft
        ]
        ...
        assert result == True
        assert mock_session.api_request.call_count == 4
    ...
    def test_send_email_cleanup_on_failure(self, sender, mock_session, mock_crypto):
        ...
        assert mock_session.api_request.call_count >= 5
